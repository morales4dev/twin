...
4. **Python routing.** Parse the Turn A label in Python. Do **not** let Turn A write the visitor-facing reply. Only a parsed `IN_SCOPE` starts **Turn B**. When Turn B does not run (`OUT_OF_SCOPE`, any other token, empty/unparseable output, or a classifier error) and this turn had a typed email, Python returns a **LEAD_ACK** string that interpolates the **extracted** address and still skips Turn B. When Turn B does not run and this turn had **no** typed email, fail closed to the canned refusal. Ingress flow diagram: `design.md`.
...
No **BREAKING** API change. Visitor-facing chat stays the same Gradio UI. Firewall-blocked turns still look like the canned refusal (including when an email was silently recorded). Out-of-scope or fail-closed turns that included a typed email look like a Python lead acknowledgement, not a career answer and not the canned refusal.

## Capabilities

### New Capabilities

- `prompt-firewall`: Code-side deny-list on the latest user message. On match, skip MiniMax entirely (no Turn A, no Turn B), return the canned refusal, and still capture a typed email when present (silent: no lead acknowledgement).
- `scope-classifier`: Stateless binary Turn A (`IN_SCOPE` | `OUT_OF_SCOPE`) on delimited current-user text only, with tools omitted on that API call; Python fail-closed routing (never classify-and-answer in one completion); when Turn B does not run and this turn had a typed email, Python LEAD_ACK instead of the canned refusal; Turn B is the existing twin completion with tools and Gradio history.
...
- **UX:** Same Gradio `ChatInterface`. Extra latency/cost only when the firewall does not match (one classifier call, then maybe Turn B). Lead-only / out-of-scope turns with a typed email get a Python acknowledgement instead of the canned refusal. Firewall hits never acknowledge a captured email.
- **Tests/docs:** Unit tests for firewall, parser, fail-closed, mixed-turn, typed-email capture, and LEAD_ACK routing. Jose battery remains evidence, not an automated suite in v1.
